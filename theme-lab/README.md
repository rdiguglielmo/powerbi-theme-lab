...
For what this folder is doing in a repository with no report in it, see
[`../README.md`](../README.md); for the diagrams of the whole pipeline,
[`../HOW-IT-WORKS.md`](../HOW-IT-WORKS.md).

> **There is no Power BI report in this repository, and that changes how to read this file.**
> What is here is the compiler — `tokens.mjs` and `theme.mjs` — plus the gallery of what it
> produced. `build.mjs` and `harness.mjs` are here too, but they rewire a report, so pointed at
> nothing they stop with `no PBIR report at ...` and write nothing.
>
> So: **from section 4 onward, every command is one you run in your own project**, not in this
> checkout. Sections 1, 2, 3 and 7 stand on their own, and section 9 is the walkthrough for
> bringing your own PBIP.
>
> One consequence worth naming early. The scripts look for the config at `../project.config.mjs`
> — one folder up from `theme-lab/`, wherever that is. Section 9 recommends putting both inside a
> `powerbi/` folder, which is why paths like `powerbi/project.config.mjs` appear further down.
> Here there is no `powerbi/` folder, because there is no report to put beside it, so the config
> sits at the repository root — still one level up. Both are the same rule.
>
> The values it ships with describe the report in the screenshots. Treat them as a worked
> example, not as something to preserve.
...
| Colours inside a DAX-generated image | Those are text inside a formula; the theme has no way in | See [section 11](#11-keep-it-simple-what-not-to-build) |
...
One function that takes a token set and returns the theme JSON:

```js
import { buildTheme } from './theme.mjs';
const theme = buildTheme(tokens);   // tokens is one entry from tokens.mjs
```

`build.mjs` is the only caller. It picks which token set to pass by looking up `theme` in the
config — or whatever `--use` overrode it with — among the keys `tokens.mjs` exports, so the
argument is always one of the six token objects and never something assembled on the fly. The
return value is the complete theme JSON, ready to write to disk; `theme.mjs` does no I/O and
imports nothing from outside this folder.

This is where every Power BI property name lives.
...
Two of its keys are read by nothing in this folder. `parquetDir` is for the small path-fixing
script of section 9, step 3 — yours to write, and the only consumer it has. `semanticModelDir`
has no consumer at all right now; it is there because the model folder is part of a project's
wiring and the point of this file is that the wiring lives in one place. If you are copying the
folder and want the shortest config that works, you can leave it out.

### What is NOT in this folder

Page generators — the scripts that decide which pages exist and what is on them. They are yours to
write: which pages a report has is the report. They live in `../pages/`, and
[section 10](#10-where-the-report-itself-gets-designed) shows what one looks like.

**`pages/` is committed here empty, on purpose.** It marks the slot so the folder in these
instructions is not one you have to imagine — but what goes in it is yours, and no generator of
this report's pages is published anywhere.

Once you have more than one, they will want a shared helper — call it `pages/_kit.mjs` — holding
the column grid, the vertical band heights and the PBIR schema pins, so three generators do not
carry three copies of the same forty lines. That file is the deliberate opposite of this folder: it
is where everything report-specific is allowed to live. `harness.mjs` is a working template for
both, and the one rule it keeps is the one that matters: **a page generator sets no colours.**
...
`perf-light-42c9ff4b.json`, into `<name>.Report/StaticResources/RegisteredResources/` — the
folder a PBIP report keeps its registered assets in. The suffix is eight hex characters from
`randomBytes(4)`: a fresh random value on every run, not a hash of anything and not a version
number.
...
`C:\path\to\your-repo\output\parquet\`. There is no way around this: Power Query has no way to
ask "which folder am I in?" — the function simply does not exist. So that one value **does**
break when the project moves.

**The fix is a script you write once**, and it is a short one — it is not part of `theme-lab/`,
because only your project knows it has Parquet files at all. Everything it needs to do:

1. Work out where it is sitting on disk, from its own module URL.
2. Build the absolute path to your data folder from that.
3. Rewrite the single `expression` line in `<name>.SemanticModel/definition/expressions.tmdl`.

Run it with Power BI Desktop **closed**, and have it report and change nothing when the path is
already right, so it is safe to run any time. Thirty lines of `node:fs` and a regular expression.

**Do not commit the result and forget about it.** That one line is the only machine-specific value
in the project, and it is in a tracked file by necessity — the model will not open without it. Use
a placeholder in anything you publish.
...
**File:** `powerbi/project.config.mjs` — the copy you placed in step 2. Find the `theme:` line:
...
**What can go there?** Only a theme that exists. To see the list, open `theme-lab/tokens.mjs` and
find the `themes` export at the bottom of the file:
...
The valid values are the keys — as shipped: `perf-light`, `perf-dark`, `orchard`, `ledger`,
`graphite` and `cypress`. `build.mjs` derives its own list from those same keys, so adding a theme
there is the only registration step; there is no second list to keep in step. Type something that
is not in the list and the build stops immediately and tells you, rather than producing a broken
report.
...
`screenshot-all` captures every page; `screenshot <page-id>` captures one. Your page ids are the
folder names under `<name>.Report/definition/pages/`, and `pages.json` lists them in display order
— worth writing down once rather than digging for them on every pass.
...
accident. **It does change the working tree.** Every build, `--use` or not:

- writes a new theme file under a fresh GUID and **deletes the previous one**
- rewrites all three theme references in `report.json` — the fourth of the four places in
  step 4 is the theme file's own internal `name`, which is written with the file itself
...
> report then points at a theme nobody else can load. It is an easy mistake and a silent one.
> Two lines settle it, run in the repository that holds your report, and they must name the
> same file:
>
> ```bash
...
```bash
node pages/01-your-page.mjs              # generator FIRST
...
This section exists because this was built more complicated than it needed to be once, and the
detour is worth writing down so nobody repeats it.
