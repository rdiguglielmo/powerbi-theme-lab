...
if (!existsSync(RESOURCES)) {
  console.error(`no PBIR report at ${REPORT}`);
  console.error('build.mjs rewires a report; it needs one. Check `reportDir` in project.config.mjs.');
  process.exit(1);
}

// Remove every previously generated theme, not just older builds of this id. Only one
// custom theme can be registered at a time, so leaving the others on disk just
